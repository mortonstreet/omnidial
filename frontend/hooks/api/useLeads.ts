...
export function useLeads(params: ListLeadsParams = {}) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (params.inPipeline) searchParams.set('inPipeline', 'true')
      if (params.includeDeleted) searchParams.set('includeDeleted', 'true')
      if (params.includeClient) searchParams.set('includeClient', 'true')
      ...
      if (params.createdAtFilter)
        searchParams.set(
          'createdAtFilter',
          JSON.stringify(params.createdAtFilter),
        )
...
export function useLead(id?: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      return await get<Lead>(
        `${ENDPOINTS.LEADS.GET(id!)}?organizationId=${orgId}`,
      )
    },
    enabled: !!id && !!orgId,
  })
...
/**
 * Resolve a phone number to a saved lead — used to put a name on inbound calls
 * instead of "Unknown Caller".
 *
 * The endpoint 404s for numbers that aren't in the CRM, which is a normal
 * outcome here rather than an error, so that case resolves to `null` and
 * retries are disabled (a genuinely unknown caller would otherwise be retried
 * on every ring). The lookup tolerates any formatting — the server strips
 * punctuation and matches on the last 10 digits — so a raw "12126804902"
 * off the WebRTC event works without normalizing it first.
 */
export function useLeadByPhone(phone?: string | null) {
  ...
  return useQuery({
    queryKey: QUERY_KEYS.leadByPhone(phone ?? undefined),
    queryFn: async () => {
      try {
        return await get<Lead>(
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({
        predicate: (query) =>
          query.queryKey[0] === 'list' || query.queryKey[0] === 'listLeads',
      })
    },
  })
}
...
export function useBulkAddToCampaign() {
  ...
  return useMutation({
    mutationFn: async (params: { leadIds: string[]; campaignId: string }) => {
      return post<BulkAddToCampaignResponse>(
        ENDPOINTS.LEADS.BULK_ADD_TO_CAMPAIGN,
        {
          ...params,
          organizationId: orgId,
        },
...
export function useBulkAddToPipeline() {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      return post<BulkAddToPipelineResponse>(
        ENDPOINTS.LEADS.BULK_ADD_TO_PIPELINE,
        {
          ...params,
          organizationId: orgId,
        },
