...
export function ParallelDialerControls({
  ...
  const [currentAttempts, setCurrentAttempts] = useState<
    ParallelDialAttemptResponse[]
  >([])
  const [connectedAttempt, setConnectedAttempt] =
    useState<ParallelDialAttemptResponse | null>(null)
  ...
  const { data: dialableNumbers, isLoading: isLoadingNumbers } =
    useDialablePhoneNumbers(orgId)
  const { data: sessionData } = useParallelDialSession(currentSessionId)

  const assignedCallerIdCount = dialableNumbers?.length ?? 0
  const hasEnoughAssignedCallerIds = assignedCallerIdCount >= lineCount
  ...
  const handleStart = async () => {
    console.log('[ParallelDialer] Starting session with:', {
      campaignId,
      listId,
      lineCount,
    })
    try {
      ...
      if (result.conferenceId) {
        ...
        try {
          ...
          console.log(
            '[ParallelDialer] Conference join initiated, waiting for connection...',
          )
          onSessionStart?.(result.id, result.conferenceId)
          toast.success(
            'Parallel dial session started - connecting to conference...',
          )
        } catch (conferenceError) {
          console.error(
            '[ParallelDialer] Failed to join conference:',
            conferenceError,
          )
          toast.error(
            'Failed to connect to conference. Check browser console for details.',
          )
      ...
      if (message.includes('already has an active')) {
        toast.error(
          'You already have an active parallel dial session. Please end it first.',
        )
      } else {
        toast.error(message)
      }
  ...
  const handleDialNextBatch = async () => {
    console.log('[ParallelDialer] handleDialNextBatch called', {
      currentSessionId,
      isInConference,
    })
    if (!currentSessionId) {
      console.error('[ParallelDialer] Cannot dial - missing sessionId', {
        currentSessionId,
      })
      return
    }
  ...
  const hasActiveCall = currentAttempts.some((a) =>
    ['dialing', 'ringing', 'connected'].includes(a.status),
  )
  const abandonRate =
    sessionStats.attempts > 0
      ? ((sessionStats.abandoned / sessionStats.attempts) * 100).toFixed(1)
      : '0'
  const canDialBatch =
    isInConference && !isDialing && !hasActiveCall && hasEnoughAssignedCallerIds
      ...
      <div className="flex items-center justify-between p-4 border-b border-border">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <button
              onClick={() =>
                !isSessionActive && setShowLineSelector(!showLineSelector)
              }
        ...
        <div className="p-4">
          ...
          <div className="mb-4 rounded-md border border-border bg-muted px-3 py-2">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="text-xs text-muted-foreground">
                {isLoadingNumbers
                  ? 'Loading...'
                  : `${assignedCallerIdCount} assigned`}
              </span>
            </div>
            {!isLoadingNumbers && !hasEnoughAssignedCallerIds && (
              <p className="mt-1 text-xs text-amber-500">
                Assign at least {lineCount} numbers to use {lineCount} parallel
                lines.
              </p>
            )}
          </div>
          ...
          <div className="flex items-start gap-2 p-3 bg-amber-500/10 border border-amber-500/20 rounded-md mb-4">
            ...
            <p className="text-xs text-amber-500">
              Abandoned calls will be tracked for compliance. Ensure you have
              proper consent to call these leads.
            </p>
          </div>
          ...
          <div className="divide-y divide-border">
            {currentAttempts.length > 0
              ? currentAttempts.map((attempt, index) => {
                  const status =
                    statusConfig[attempt.status as AttemptStatus] ||
          ...
          <div className="p-3 bg-muted/50 border-t border-border">
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              ...
              <span className="text-green-500">
                {sessionStats.connects} connected
              </span>
