...
function _StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex px-2 py-0.5 text-xs font-medium rounded-full ${colors[status] || 'bg-gray-100 text-gray-700'}`}
    >
      {status}
    </span>
  )
        ...
        <div className="space-y-6 max-h-[70vh] overflow-y-auto">
          ...
          <div>
            ...
            <div className="bg-muted/50 rounded-lg p-3 space-y-0">
              ...
              <InfoRow label="Telnyx SID" value={call.twilioCallSid} />
              ...
              {(call.leadFirstName || call.leadLastName) && (
                <InfoRow
                  label="Lead"
                  value={`${call.leadFirstName || ''} ${call.leadLastName || ''}`.trim()}
                />
              )}
              ...
              <InfoRow
                ...
                value={
                  call.startedAt
                    ? format(parseISO(call.startedAt), 'MMM d, yyyy h:mm:ss a')
                    : null
                }
              />
              <InfoRow
                ...
                value={
                  call.answeredAt
                    ? format(parseISO(call.answeredAt), 'MMM d, yyyy h:mm:ss a')
                    : null
                }
              />
              <InfoRow
                ...
                value={
                  call.endedAt
                    ? format(parseISO(call.endedAt), 'MMM d, yyyy h:mm:ss a')
                    : null
                }
              />
            </div>
            ...
            <div>
              <h4 className="text-sm font-medium text-foreground mb-2">
                ...
                <span className="text-xs text-muted-foreground ml-2">
                  ({formatDuration(call.transcript.durationSeconds)} /{' '}
                ...
                <div className="mb-2">
                  ...
                  <ul className="text-sm text-muted-foreground space-y-0.5">
                    {call.coaching.strengths.map((s, i) => (
                      <li
                        key={i}
                        className="pl-3 relative before:content-[''] before:absolute before:left-0 before:top-2 before:w-1.5 before:h-1.5 before:rounded-full before:bg-green-400"
                      >
                        {s}
                      </li>
                    ))}
                ...
                <div>
                  ...
                  <ul className="text-sm text-muted-foreground space-y-0.5">
                    {call.coaching.improvements.map((s, i) => (
                      <li
                        key={i}
                        className="pl-3 relative before:content-[''] before:absolute before:left-0 before:top-2 before:w-1.5 before:h-1.5 before:rounded-full before:bg-amber-400"
                      >
                        {s}
                      </li>
                    ))}
